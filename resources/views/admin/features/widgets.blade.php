{{--
    Every Backpack widget type, on one page: progress, card, alert, chart (with several chart libraries).
    This used to be the demo dashboard; the dashboard is now a realistic Pet Shop one.
--}}
@extends(backpack_view('blank'))

@section('header')
    @include('admin.partials.feature_header', [
        'title'       => 'Widgets',
        'description' => ($description ?? '').' Add them to any page, from the controller or the view, with a fluent syntax or plain arrays.',
        'docs'        => 'https://backpackforlaravel.com/docs/7.x/base-widgets',
    ])
@endsection
...
@php
 ...
 //    ])->to('before_content')->makeFirst();
    ...
    $widgets['after_content'][] = [
      ...
      'heading'      => 'Alert widget',
      'content'      => 'Any Bootstrap alert class works: alert-info, alert-success, alert-warning, alert-danger. With or without a close button.',
